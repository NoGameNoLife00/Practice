...
for_each_trigram_in_the_english_language(F, A0) ->
    {ok, Bin0} = file:read_file("354984si.ngl"),
...
get_next_word([], L) -> {lists:reverse([$\s|L]), L}.
...
make_a_set(Type, FileName) ->
    ...
    F = fun(Str, _) ->
        ets:insert(Tab, {list_to_binary(Str)}) end,
...
lookup_all_ets(Tab, L) ->
    list:foreach(fun({K}) -> ets:lookup(Tab, K) end, L).
...
open() ->
    File = filename:join(filename:dirname(code:which(?MODULE)), "/trigramS.tab"),
    {ok, Tab} = ets:file2tab(File),
    Tab.
